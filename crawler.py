...
class Crawler:
    def __init__(self, classifier, target_label, min_prob, query, num_tweets, max_queue_size=1000):
    ...
    def step(self):
        ...
        if len(self.queue) != 0:
            ...
            if current.username not in self.already_checked:
                ...
                probs = self.classifier.predict(str(current))

                if probs[self.target_label] > self.min_prob:
